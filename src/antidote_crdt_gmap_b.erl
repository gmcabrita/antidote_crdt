...
-type gmap() :: #{{Key::term(), Type::atom()} => NestedState::term()}.
-type gmap_op() ::
    {update, nested_op()}
  | {update, [nested_op()]}
  | {reset, {}}.
...
value(Map) ->
  lists:sort([{{Key, Type}, Type:value(Value)} || {{Key, Type}, Value} <- maps:to_list(Map)]).
...
downstream({update, {{Key, Type}, Op}}, CurrentMap) ->
    ...
    CurrentValue = case maps:is_key({Key, Type}, CurrentMap) of
      true -> maps:get({Key, Type}, CurrentMap);
      false -> Type:new()
    end,
...
downstream({reset, {}}, CurrentMap) ->
  ...
  DownstreamResets = lists:flatmap(Reset, maps:to_list(CurrentMap)),
  {ok, {update, DownstreamResets}}.

-spec update(gmap_effect(), gmap()) -> {ok, gmap()}.
update({update, {{Key, Type}, Op}}, Map) ->
    case maps:is_key({Key, Type}, Map) of
      true ->  V = maps:get({Key, Type}, Map),
               {ok, Value} = Type:update(Op, V),
               {ok, maps:update({Key, Type}, Value, Map)};
      false -> NewValue = Type:new(),
               {ok, NewValueUpdated} = Type:update(Op, NewValue),
               {ok, maps:put({Key, Type}, NewValueUpdated, Map)}
    end;
